...
| **Rank re-check (Rank Tracking → Re-check)** | One standard-queue SERP task per keyword (Google or Bing endpoint by entry engine, city-targeted, device-aware, depth 100) parsed with `parseSerpRank` — the true `rank_absolute`. $0.0006 per keyword (vs $0.003 live); a 25-keyword batch returns in ~1–5 min. | same |
