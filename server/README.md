...
| **GBP geo-grid rank tracker** (Performance Studio → GBP Rank Tracking) | One coordinate-targeted Google Maps SERP task per grid point (`location_coordinate: lat,lng,17z`, depth 100, via DataForSEO's standard task queue: `task_post` → `tasks_ready` → `task_get/advanced`) — each point returns what a searcher at that exact spot sees; 17z keeps neighboring points differentiated. Business matched by place_id/CID first, normalized name second. Circle grids are radial (center + rings every `spacing`, ring k carrying ⌊2πk⌋ pins). Location resolved via the real Google Places Find Place API (or manual coordinates). Cost: points × keywords tasks @ $0.0006 per scan (vs $0.0035 live) — all queued in one batch, results within ~1–5 min. | DataForSEO (scans) + optional Google Places API key (location lookup) |
...
| **Google Business Profile performance** (Performance Studio → Overview + Business Profiles) | `POST /api/gbp/performance` → the Business Profile **Performance API** (`locations/{id}:fetchMultiDailyMetricsTimeSeries`): daily impressions (Maps/Search × desktop/mobile), calls, website clicks, direction requests, messages and bookings for the listing connected to each location group. Folded into the dashboards' 13-month grid, so the existing date-range presets and custom range work unchanged. **Not** read from GA4: Google confines Business Profile metrics to GA4's Standard Reports UI (no Data API, no Explorations) and keeps only a rolling 6 months — the source API serves ~18. Unapproved API → 502 naming the quota problem; no listing → the honest requirements panel. | Google OAuth app + `business.manage` consent, and the Business Profile APIs approved in Cloud Console |
| **Rank re-check (Rank Tracking → Re-check)** | One standard-queue SERP task per keyword (Google or Bing endpoint by entry engine, city-targeted, device-aware, depth 100) parsed with `parseSerpRank` — `rank_group`, the ORGANIC position (what every rank tracker reports); `rank_absolute` is kept alongside as `absPos`. The SAME response also yields the local map-pack position (`mapPos`) and the Google AI Overview citation position (`aiPos` — the domain's 1-based place among the overview's cited references; cached/synchronous overviews only, so zero extra cost — `load_async_ai_overview` is deliberately not sent because it doubles the task price whenever an async overview exists). $0.0006 per 10 results, so $0.006 per keyword at depth 100; a 25-keyword batch returns in ~1–5 min. A task handed out twice (40601) is re-queued at the same price, never re-run on the live endpoint. | same |
