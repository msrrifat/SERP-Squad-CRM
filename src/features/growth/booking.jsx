          ...
          {sorted.map((b) => (
            <button key={b.id} onClick={() => setOpen(b)} className="flex w-full flex-wrap items-center gap-2 rounded-xl border border-gray-100 p-3 text-left hover:border-gray-200">
              ...
              <span className="ml-auto flex items-center gap-2 text-[11px]">
                ...
                <span onClick={async (e) => { e.stopPropagation(); if (await askDelete("this booking")) commit({ bookings: bookings.filter((x) => x.id !== b.id) }); }} className="rounded-md p-1 text-gray-300 hover:text-red-500"><Trash2 size={13} /></span>
              </span>
            </button>
          ))}
        </div>
