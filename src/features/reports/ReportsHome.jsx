...
import { Card, askDelete } from "../../ui/primitives.jsx";
            ...
            {templates.map((t) => (
              <Card key={t.id} className="group flex flex-col p-4">
                <div className="flex items-start gap-2">
                  ...
                  <button onClick={() => askDelete(`the template "${t.name}"`).then((ok) => { if (ok) onDeleteTemplate(t.id); })} className="text-gray-300 opacity-0 hover:text-red-500 group-hover:opacity-100"><Trash2 size={13} /></button>
                </div>
          ...
          <div className="space-y-2">
            {savedReports.map((r) => (
              <Card key={r.id} className="group flex items-center gap-3 p-3.5">
                ...
                <button onClick={() => askDelete(`the saved report "${r.name}"`).then((ok) => { if (ok) onDeleteReport(r.id); })} className="text-gray-300 opacity-0 hover:text-red-500 group-hover:opacity-100"><Trash2 size={14} /></button>
              </Card>
            ))}
          </div>
        )}
