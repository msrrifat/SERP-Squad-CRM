...
export const DFS_RATES = {
  organic:      { rate: 0.002,  extra: 0.0015,  label: "Google organic live-advanced" },
  bing:         { rate: 0.002,  extra: 0.0015,  label: "Bing organic live-advanced" },
  maps:         { rate: 0.0035, extra: 0.0035,  label: "Google Maps live-advanced" },
  organicQueue: { rate: 0.0006, extra: 0.00045, label: "Google organic standard queue" },
  bingQueue:    { rate: 0.0006, extra: 0.00045, label: "Bing organic standard queue" },
  mapsQueue:    { rate: 0.0006, extra: 0.0006,  label: "Google Maps standard queue" },
};

/* how many 10-result pages a depth asks for */
export const depthUnits = (depth) => Math.max(1, Math.ceil((+depth || 10) / 10));
/* what ONE task at this depth costs */
export const taskCost = (kind = "organic", depth = 10) => {
  const r = DFS_RATES[kind] || DFS_RATES.organic;
  return r.rate + (depthUnits(depth) - 1) * (r.extra ?? r.rate);
};
export const dfsCost = (requests, kind = "organic", depth = 10) => requests * taskCost(kind, depth);
...
export function DfsCostChip({ requests, kind = "organic", depth = 10, className = "" }) {
  ...
  return (
    <span className={"ll-mono inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[9.5px] font-bold text-amber-700 " + className}
      title={`Estimated DataForSEO cost: ${requests.toLocaleString()} request${requests === 1 ? "" : "s"} × $${taskCost(kind, depth).toFixed(5)} (${r.label}${units > 1 ? `, top ${units * 10} results: $${r.rate} for the first 10 + ${units - 1} × $${r.extra ?? r.rate}` : ""}). A keyword DataForSEO has run recently cannot be re-queued and is re-checked live, which costs more.`}>
